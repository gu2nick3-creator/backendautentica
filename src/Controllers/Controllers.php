...
class AuthController extends BaseController {
    public function register(Request $r, array $p): void {
        ...
        $token = (new JwtService())->encode(['sub' => $user['id'], 'role' => $user['role'], 'exp' => $exp]);
    ...
    public function login(Request $r, array $p): void {
        $d = $r->input();
        $repo = new UserRepository();
        $user = $repo->findByEmail((string)($d['email'] ?? ''));
...
class ProductController extends BaseController {
    ...
    public function show(Request $r, array $p): void {
        ...
        if (!$item) Response::error('Produto não encontrado', 404);
        Response::ok($item);
    }

    public function store(Request $r, array $p): void {
        $d = $r->input();
        if (empty($d['name'])) Response::error('Nome do produto é obrigatório', 422);
...
class CategoryController extends BaseController {
    ...
    public function store(Request $r, array $p): void {
        $d = $r->input();
        if (empty($d['name'])) Response::error('Nome da categoria é obrigatório', 422);
        $repo = new CategoryRepository();
        $id = $repo->create($d);
        $created = array_values(array_filter($repo->all(), fn($c) => (string)$c['id'] === (string)$id));
    ...
    public function update(Request $r, array $p): void {
        $repo = new CategoryRepository();
        $repo->update($p['id'], $r->input());
        $updated = array_values(array_filter($repo->all(), fn($c) => (string)$c['id'] === (string)$p['id']));
...
class CouponController extends BaseController {
    ...
    public function validate(Request $r, array $p): void {
        ...
        if ($code === '') Response::error('Informe um cupom', 422);
        $coupon = (new CouponRepository())->findActiveByCode($code);
        if (!$coupon) {
            Response::ok(['valid' => false]);
        }
        Response::ok(['valid' => true, 'coupon' => $coupon]);
    }
...
class OrderController extends BaseController {
    ...
    public function show(Request $r, array $p): void {
        ...
        if (!$isAdmin && !$isOwner) Response::error('Sem permissão para acessar este pedido', 403);
        Response::ok($order);
    }
    ...
    public function store(Request $r, array $p): void {
        ...
        if (!empty($d['couponCode'])) {
            ...
            if ($coupon) $repoCoupon->incrementUse((string)$coupon['code']);
        }
    ...
    public function updateStatus(Request $r, array $p): void {
        ...
        if ($status === '') Response::error('Status é obrigatório', 422);
        $repo = new OrderRepository();
    ...
    public function updateTracking(Request $r, array $p): void {
        ...
        if ($tracking === '') Response::error('Código de rastreio é obrigatório', 422);
        $repo = new OrderRepository();
...
class UploadController extends BaseController {
    public function store(Request $r, array $p): void {
        ...
        if (!$file) Response::error('Arquivo não enviado', 422);
        if (($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) Response::error('Falha no upload', 422);
        if (($file['size'] ?? 0) > (int) env('UPLOAD_MAX_SIZE', '10485760')) Response::error('Arquivo acima do limite', 422);
        $mime = mime_content_type($file['tmp_name']);
        $ext = match($mime){'image/jpeg'=>'jpg','image/png'=>'png','image/webp'=>'webp',default=>''};
        if ($ext === '') Response::error('Formato inválido', 422);
        if (!is_dir(base_path('public/uploads'))) mkdir(base_path('public/uploads'), 0777, true);
